'''https://github.com/hankcs/udacity-deep-learning/blob/master/4_convolutions.py'''
...
with graph.as_default():
    ...
    # Model.
    def model(data):
        conv = tf.nn.conv2d(data, layer1_weights, [1, 2, 2, 1], padding='SAME') #computes 2D convolution with 4D input and filter tensors
        #1. flatten filter to a 2D matrxi, 2. Extracts patches from the tensor. 3. for each multiples filter mat with image patch. Returns a tensor like the input
        hidden = tf.nn.relu(conv + layer1_biases) #rectified finear: max(featurs, 0)
...
with graph.as_default():
    ...
    # Optimizer.
    learning_rate = tf.train.exponential_decay(1e-1, global_step, num_steps, 0.7, staircase=True) #Expoonential decay on learning rate from value :: decayed_learning_rate = learning_rate *   decay_rate ^ (global_step / decay_steps)
